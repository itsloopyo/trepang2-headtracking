...
<#
.SYNOPSIS
    Runs uninstall.cmd against every local install of Trepang2.
...
$paths = @(Find-AllGamePaths -GameId 'trepang2')
if ($paths.Count -eq 0) {
    Write-Host 'No Trepang2 installation found; nothing to uninstall.' -ForegroundColor Yellow
    exit 0
}
