...
try {
    ...
    $readme = @(
        ...
        '`winmm.dll` beside the game exe (`CPPFPS/Binaries/Win64/` on Steam,',
        '`CPPFPS/Binaries/WinGDK/` on Xbox / Game Pass) as the ASI hook slot: both shipping exes',
        'import WINMM.dll statically and do not import dinput8.dll, which Windows would only',
        'resolve out of System32.'
    ) -join "`n"
