...
void OpenLog() {
    ...
    // rotates the outgoing one to HeadTracking.prev.log first. It warns by
    // itself when the rotation fails, so there is nothing to do here but name
    // the file.
    Log::Open(exeDir + L"\\HeadTracking.log");
    Log::Line("=== Trepang2 Head Tracking v" T2_HT_VERSION " (UE4.27) ===");
}
