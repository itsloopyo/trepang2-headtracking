...
// A TWeakObjectPtr is an object index and a serial number, side by side.
constexpr std::size_t kWeakPointerFields = 2;
...
bool Resolve() {
    ...
    // Actor and Component are read as a TWeakObjectPtr - an object index and a
    // serial number side by side. A narrower field is not that pair, and reading
    // one as if it were takes the bytes that follow it for an object index.
    constexpr std::size_t kWeakBytes = kWeakPointerFields * sizeof(std::int32_t);
...
// The player's own gear (a held shield, a dual-wielded weapon, an attachment
// actor) is owned by the pawn and does not stop the player's rounds, so a
...
Result Cast(std::uintptr_t pawn, std::uintptr_t weapon, const ue::FVector& start, const ue::FVector& dir,
            double maxDistance) {
    Result r;
    ...
    if (weapon) g_ignoreStorage[ignored++] = weapon;
    ...
    for (int attempt = 0; attempt < kMaxIgnored; ++attempt) {
        ...
        std::int32_t weak[2 * kWeakPointerFields];
        std::memcpy(weak, hit + g_readback.HitActor, kWeakPointerFields * sizeof(std::int32_t));
        std::memcpy(weak + kWeakPointerFields, hit + g_readback.HitComponent,
                    kWeakPointerFields * sizeof(std::int32_t));
        r.Actor = ue_call::ResolveWeak(weak[0], weak[1]);
        r.Component = ue_call::ResolveWeak(weak[2], weak[3]);
