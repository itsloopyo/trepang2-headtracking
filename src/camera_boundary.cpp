...
namespace t2_ht::camera_boundary {
...
ue::FVector PositionOffset(const ue::FQuat4d& cleanRotation,
    ...
    // The processor z runs the other way to UE camera-forward: it clamps to
    // [-limit_z, +limit_z_back], i.e. NEGATIVE z is the forward lean. Negate it
    // here, at the engine boundary, rather than via the processor invert_z -
    ...
    //
    // Sway is negated for the same reason its ancestors negate it, and the
