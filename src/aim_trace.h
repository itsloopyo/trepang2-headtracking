...
//
// `pawn` and `weapon` are excluded, so the ray does not stop on the player's own
// body or gun. Must be called on the game thread.
Result Cast(std::uintptr_t pawn, std::uintptr_t weapon,
...
// gear, which neither the shot nor the lean stops on.
