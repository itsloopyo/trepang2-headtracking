...
std::int32_t g_lastHitWeak[4] = {-1, 0, -1, 0};
bool  g_ready = false;
bool  g_failed = false;
...
std::string LastHitDescription() {
    const std::uintptr_t actor = ue_call::ResolveWeak(g_lastHitWeak[0], g_lastHitWeak[1]);
    const std::uintptr_t component = ue_call::ResolveWeak(g_lastHitWeak[2], g_lastHitWeak[3]);
...
LeanObstruction Query(void*, const Vec3& start, const Vec3& direction, float maxDistance) {
    ...
    for (int attempt = 0; attempt <= kMaxIgnored; ++attempt) {
        ...
        std::memcpy(g_lastHitWeak, hit + g_readback.HitActor,
                    kWeakPointerFields * sizeof(std::int32_t));
        std::memcpy(g_lastHitWeak + kWeakPointerFields, hit + g_readback.HitComponent,
                    kWeakPointerFields * sizeof(std::int32_t));
        const std::uintptr_t actor = ue_call::ResolveWeak(g_lastHitWeak[0], g_lastHitWeak[1]);
